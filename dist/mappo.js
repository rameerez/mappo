...
function buildGlobePoints(cols, latRange, body, ground = false) {
...
function buildGlobeFlags(cols, latRange, test, body) {
  const rows = Math.round((cols / 360) * (latRange[1] - latRange[0]));
  const grid = { cols, rows, latRange };
  const out = [];
  for (let row = 0; row < rows; row++) {
...
function buildGlobePhases(cols, latRange, mode, body) {
  const rows = Math.round((cols / 360) * (latRange[1] - latRange[0]));
  const grid = { cols, rows, latRange };
  const out = [];
  for (let row = 0; row < rows; row++) {
