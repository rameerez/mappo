...
mappo's globe is an orthographic render on a flat canvas: it can draw the
Earth beautifully from outside, and it cannot put a camera on the ground.
Rather than drop mappo there, mappo draws the map and three.js wraps it round
a sphere — the cartography is still mappo's, the camera is three.js's.
